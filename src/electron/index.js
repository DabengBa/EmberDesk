...
function createEmberDeskWindow() {
    if (!appUrl) {
...
function startServer() {
    return new Promise((_resolve, _reject) => {
        serverEvents.addListener(EVENT_NAMES.SERVER_STARTED, ({ url }) => {
            appUrl = url.toString();
            createEmberDeskWindow();
        });
        const emberDeskRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../..');
        process.chdir(emberDeskRoot);
