...
|---|---|---|---|---|---|---|---|
| Same-entry legacy workspace chrome for `/` | `retired` | React workspace shell with declared child slots | prior-version deployment only | `bun run --cwd tests test:e2e -- workspace-shell-panel-navigation.e2e.js third-party-extension-runtime.e2e.js --workers=1`; `bun run --cwd tests test:unit -- react-workspace-panels-helpers.test.js react-state-stores.test.js --runInBand` |  | `.docs/db/features/next-workspace-shell.md`; `.docs/db/pages/chat-workspace.md`; `.docs/tech/workspace-shell-panel-dock-coordination.md` | 2026-07-18 |
...
| Main-chat message rows, rich body, actions, mutation zones, windowing, and reading-position restore | `sole-owner` | React main-chat message-list controller + framework-neutral `chat-message-render-service`; product `mainChatMessageList` flag retired | previous application version deploy (no product flag-off legacy list owner) | `bun run test:compat`; `bun run --cwd tests test:e2e -- chat-message-rendering.e2e.js chat-message-layout.e2e.js chat-message-list-walkthrough.e2e.js chat-message-streaming.e2e.js --workers=1`; `bun run --cwd tests test:unit -- react-workspace-panels-helpers.test.js chat-message-render-descriptor.test.js chat-message-render-service.test.js --runInBand`; `bun run perf:interaction` |  | `.docs/db/features/chat-message-rendering.md`; `.docs/db/pages/chat-workspace.md`; `.docs/tech/main-chat-rendering-call-chain.md`; `.docs/adr/0012-react-migrated-surface-legacy-retirement.md` | 2026-07-18 |
| `printMessages` / `showMoreMessages` / `updateMessageElement` / `addOneMessage` public aliases and DOM producers | `freeze-supported` | thin compatibility adapters that feed React-owned rows and React load-earlier policy; not product dual-owner surfaces | prior application version deploy; documented public names remain callable | `bun run test:compat`; `bun run --cwd tests test:unit -- chat-workspace-structure.test.js chat-message-render-service.test.js --runInBand`; `bun run --cwd tests test:e2e -- chat-message-rendering.e2e.js --workers=1` |  | `.docs/db/features/chat-message-rendering.md`; `.docs/tech/third-party-extension-compatibility.md` | 2026-07-18 |
