...
状态：Phase 0-6 和原 Phase 7 已交付为 React guarded-island、状态和兼容性基线。2026-07-16 起，已迁移 React surface 进入新的 legacy-retirement program；2026-09-15 的 Scope B 收敛已将 global Background Library、Expressions、waifu controls 和 global sprite persistence 标记为 retired。当前保留的是 active chat background、`custom_background` / `chat_backgrounds` metadata、静态 `backgrounds/*` compatibility、avatar/persona media 与群聊 `hideMutedSprites`，不是一个 global gallery。该方向由 [ADR-0012](../adr/0012-react-migrated-surface-legacy-retirement.md) 和 [Legacy Cutover Ledger](legacy-cutover-ledger.md) 记录。仍不引入 `/workspace-next`，不把未迁移 surface 纳入，也不以删除为名缩减用户能力。
创建日期：2026-06-15  
...
2. complete existing foundations: Character Library, Character/Group Authoring, Settings, and World Info; the global Background Library is now retired;
3. complete cross-cutting extension/automation contracts while each retained foundation moves;
...
**目标**：迁移角色库、世界信息、扩展宿主面板等主工作区侧边栏/面板到 React；global Background Library 已在后续 retirement wave 退役，active chat background 不属于该 panel migration。
...
- ✅ Sprint 6: 背景库面板（2 周，历史 guarded React status/filter/gallery/action island；已被 2026-09-15 global Background Library retirement superseded）
...
**Phase 1 Sprint 3 后续边界**：`World Info`、`Backgrounds`、`Extensions` 没有混入 React `/settings`。这条记录描述历史 Phase 2 排期；当前 global Background Library 已 retired，active chat background 属于 Chat Workspace 保留契约，Extensions drawer 宿主仍由 Phase 2/兼容文档维护。

**Sprint 4-7 当前边界**：
- `Sprint 1-5 and 7` remain the current Phase 2 panel migration record; the Background Library island, gallery host and legacy owner descriptions below are historical traceability only and are superseded by the 2026-09-15 retirement.
- `public/scripts/workspace-panels-react-bridge.js` 的历史 flag/host behavior 记录保留用于 traceability；当前 Background Library flag、host、gallery 和 action owner 已 retired，不能被解释为可用 fallback。
- `public/scripts/workspace-panel-host-controller.js` 的历史 Background Library host-lifecycle 记录保留；当前 shell 只协调仍有导航入口的 panels，并不创建 Backgrounds host。
...
2028 Q3-Q4 historical Phase 7 schedule (superseded by delivered cutover records)：Phase 7 full owner cutover and legacy fallback retirement
  ├─ Sprint 1-2: Character Library / World Info full owner cutover
  ├─ Sprint 3: global Background Library retirement (completed 2026-09-15)
...
- Any future work that reopens character library, World Info, the retired global Background Library, extensions host, main-chat transport, main-chat renderer/windowing, workspace shell, or global compatibility exports must start from the completed Phase 7 ADR closeout and the 2026-09-15 media-retirement decision instead of treating those owner splits as unfinished roadmap debt.
...
- [Background Library Panel (Retired)](../db/features/background-library-panel.md) - retired global gallery boundary; active chat background and avatar/media retention are documented separately
