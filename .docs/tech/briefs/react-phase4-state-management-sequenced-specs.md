...
Phase 0-3B 已完成并从 active specs 中清理，Phase 4 现在是下一批 React 现代化开发入口。用户希望 Phase 4 不再停留在目录级占位，而是拆成可审批、可执行、可交付的分步骤 `spec.md`，并保持 roadmap 中对 Zustand、global compatibility bridge、extension guide、Phase 4A transport expansion 和 Phase 4B renderer extraction 的边界。
...
- **Implementation traceability:** delivered via `app/stores/workspace-panel-store.js`, `app/stores/main-chat-observation-store.js`, `app/workspace-panels.tsx`, `tests/react-state-stores.test.js`; delivery status `delivered`.
...
- **Implementation traceability:** delivered via `app/compat/global-compatibility-bridge.js`, `app/workspace-panels.tsx`, `tests/global-compatibility-bridge.test.js`; delivery status `delivered`.
...
- **Implementation traceability:** delivered via `.docs/tech/third-party-extension-compatibility.md` and `tests/third-party-extension-compatibility.test.js`; delivery status `delivered`.
...
- **Implementation traceability:** delivered via `public/scripts/chat-message-render-descriptor.js`, `.docs/tech/main-chat-rendering-call-chain.md`, `tests/chat-message-render-descriptor.test.js`, `tests/chat-message-rendering.e2e.js`, `tests/chat-message-layout.e2e.js`; delivery status `delivered`.
