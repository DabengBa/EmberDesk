...
- **实现追踪**：第一到第五阶段均已在当前 `/` 路径交付，未引入 `/workspace-next`；持久事实见 `.docs/PROJECT_HISTORY.md` 的 2026-07-01 same-entry shell takeover foundation / React workspace chrome / React main-chat layout shell，以及 2026-07-05 React workspace panel dock coordination 与 React workspace responsive and recovery hardening 记录。
...
- **当前状态**：第一阶段 foundation、第二阶段 React chrome、第三阶段 panel dock coordination、第四阶段 main-chat layout/composer shell、第五阶段 responsive/recovery hardening 已通过 focused unit、compat、React workspace panel build、semantic docs build/check、layout E2E、streaming E2E，以及 takeover=true 的真实浏览器 smoke；仅第六阶段 ADR gate 保持为后续决策输入。
- **变更历史**：
  - 2026-07-01: 第一阶段 review 修复了 failure vocabulary 过窄、strict mode 未进入 bootstrap payload、早期 body 缺失会抛原始错误的问题。
- **实现追踪**：验证面包括 `workspace-react-panel-flags.test.js`、`react-state-stores.test.js`、`global-compatibility-bridge.test.js`、`react-workspace-panels-helpers.test.js`、`chat-workspace-structure.test.js`、`world-info-card-rendering.test.js`、`character-list-structure.test.js`、`main-chat-visible-transport-owner.test.js`、`bun run test:compat`、`bun run build:react:workspace-panels`、`bun run docs:build` / `bun run docs:check`、`PLAYWRIGHT_CHROME_EXECUTABLE=/home/allen/.cache/ms-playwright/chromium-1228/chrome-linux64/chrome PLAYWRIGHT_REUSE_SERVER=0 PLAYWRIGHT_PORT=18100 bun run --cwd tests test:e2e -- chat-message-layout.e2e.js --workers=1`、`PLAYWRIGHT_CHROME_EXECUTABLE=/home/allen/.cache/ms-playwright/chromium-1228/chrome-linux64/chrome PLAYWRIGHT_REUSE_SERVER=0 PLAYWRIGHT_PORT=18101 EMBERDESK_FEATURES_REACT_PANELS_MAINCHATMESSAGELIST=true bun run --cwd tests test:e2e -- chat-message-rendering.e2e.js --workers=1`，以及 2026-07-01 takeover=true smoke（chrome/takeover status `ready`，visible legacy primary entries `0`，composer not overlapped，no `/workspace-next`）。
