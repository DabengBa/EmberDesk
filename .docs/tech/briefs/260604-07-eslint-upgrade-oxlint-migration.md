...
| ESLint major upgrade | Complete the lint-tooling upgrade rather than stopping at a fast-lint preflight | Planned | 2026-06-04: user clarified that the upgrade action must be completed; current npm latest is ESLint 10.4.1, Node 26 satisfies its engine range, and current plugin latest versions support ESLint 10 through peer ranges | Completion spec: `.docs/specs/260604-11-eslint10-flat-config-upgrade/design.md` |
