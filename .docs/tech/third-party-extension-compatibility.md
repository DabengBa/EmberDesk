...
| `#extensions_settings`, `#extensions_settings2`, `#regex_container`, `#extensionsMenuButton`, `#extensionsMenu` | `primary` | legacy extension drawer and wand menu rendering | protected legacy DOM nodes remain behavior owners when flags are off or bundle import fails | `bun run test:compat` | Sprint 4 | Static presence is covered; fail-closed host lifecycle still needs explicit Phase 6 runtime evidence notes. |
