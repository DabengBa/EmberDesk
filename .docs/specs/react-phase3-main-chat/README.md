...
1. **聊天消息列表承载层迁移**：在 guarded island 内接管消息列表承载/顺序同步，并推进 rich-body、scroll 与 headless virtual observation
2. **流式生成观察边界**：收口 streaming transport 与 visible generation control-state bridge，再把 provider transport / token append owner 留给后续独立迁移
3. **聊天输入观察边界**：观察 composer 与 slash-command state，不接管 visible textarea、send mutation、parser 或 autocomplete owner
4. **消息操作观察边界**：观察 message-action snapshots，不接管 visible action buttons 或 handlers
...
| [Phase 3 Sprint 4: 流式生成 - Transport Boundary](../../tech/briefs/react-phase3-remaining-main-chat-sprints.md#domain-sprint-4-streaming-transport-boundary) | 2 周 | hidden streamingTransport snapshot、terminal stop/error/completed replay、legacy transport/token owner 保持 | 中 |
...
| [Phase 3 Sprint 6: 输入框 - 基础功能](../../tech/briefs/react-phase3-remaining-main-chat-sprints.md#domain-sprint-6-basic-composer-bridge) | 2 周 | hidden composer snapshot、文本长度/focus/sendability 观察、legacy textarea/send owner 保持 | 低 |
| [Phase 3 Sprint 7: 输入框 - 斜杠命令](../../tech/briefs/react-phase3-remaining-main-chat-sprints.md#domain-sprint-7-slash-command-bridge) | 3 周 | hidden slashCommand snapshot、autocomplete/execution/pause/abort 观察、legacy parser/executor owner 保持 | 高 |
...
| [Phase 3 Sprint 9: 整合测试](../../tech/briefs/react-phase3-remaining-main-chat-sprints.md#domain-sprint-9-integration-closure) | 2 周 | guarded hidden-island scope closure、docs/test matrix、remaining visible-owner work 明确 | 高 |
...
Sprint 4 已收敛为 streaming transport observation boundary，而不是 SSE/EventSource rewrite：`public/script.js` 和 `StreamingProcessor` 继续拥有 provider transport、token append、stop/error/completion cleanup；React hidden controller 只消费 Zod 校验后的 `streamingTransport` payload，并通过 hidden marker 报告 phase、active message id、token/chunk count、fallback attempt 和 terminal snapshot。
...
Sprint 6 已交付 hidden composer snapshot boundary：React 观察 `#send_textarea` 的长度、empty、focus、disabled/generating、sendability 和 active context，但不复制 prompt 原文，不点击 send，不清空 textarea，不追加 user row，也不引入 TanStack Form-owned visible composer。

Sprint 7 已交付 hidden slash-command snapshot boundary：React 观察 slash active/query length、autocomplete visibility、executing、paused、aborted 和 error label，但 `public/scripts/slash-commands.js` 继续拥有 parser、registry、execution、pause/continue/abort controller、autocomplete DOM 和 protected public exports。
...
Sprint 9 已按当前批准的 guarded hidden-island scope 做整合关闭：Phase 3 完成不等于 full SPA main chat rewrite，也不等于 React-owned provider transport、visible composer、slash autocomplete/parser、visible MessageRow renderer 或 visible action menu owner。
...
- 后续可另起迁移：provider transport / token append owner、visible TanStack Form composer、visible slash autocomplete/parser UI、visible MessageRow renderer、visible action menu owner；当前 Phase 3 closure 明确不包含这些 visible-owner cutover
...
- [x] ✅ Sprint 4: hidden `streamingTransport` snapshot、terminal stop/error/completed replay、fallback attempt/token observation proof 已落地；不迁移 provider transport 或 `.mes_text` token append
...
- [x] ✅ Sprint 6: hidden composer snapshot、ordinary input、Shift+Enter、legacy send、after-send clear、empty-submit no-op、mobile reachability proof 已落地；不暴露 prompt 原文
- [x] ✅ Sprint 7: hidden slash-command snapshot、autocomplete active/inactive、legacy `/delay` execution、pause/continue/abort 和 public slash export compatibility proof 已落地；不暴露 command text/args
...
- [x] ✅ Sprint 9: 当前批准的 guarded hidden-island scope 已闭环；remaining work 是 full visible-owner migrations，不属于本次 Phase 3 closure
...
- [React Phase 3 Remaining Main Chat Sprints Intent](../../tech/briefs/react-phase3-remaining-main-chat-sprints.md)
- [Main Chat Streaming Transport Bridge Processing Flow](../../logic-description/main_chat_streaming_transport_bridge_processing_flow.md)
- [Main Chat Composer Bridge Processing Flow](../../logic-description/main_chat_composer_bridge_processing_flow.md)
- [Main Chat Slash Command Bridge Processing Flow](../../logic-description/main_chat_slash_command_bridge_processing_flow.md)

---

## 下一步

Phase 3 当前批准的 guarded hidden-island scope 已完成。下一步不应把本 README 里的旧 SSE/Input/Slash stub 直接当作 delivery 入口；如果要继续推进主聊天，应另起 spec 明确一个 visible owner cutover，例如 provider transport/token append、TanStack Form composer、slash autocomplete/parser UI、visible MessageRow renderer 或 visible action menu owner。
