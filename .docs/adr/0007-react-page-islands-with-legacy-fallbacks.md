...
2026-07-07 Legacy cutover closeout update:

- Phase 7 no longer carries an open-ended "delete the remaining legacy fallback later" backlog. The durable closeout source is the [Legacy Cutover Ledger](../tech/legacy-cutover-ledger.md), which classifies each retained workspace, authoring, World Info, Backgrounds, Extensions, main-chat, and global compatibility surface as a documented compatibility facade, a freeze-supported boundary, a blocked deletion candidate, or a deletion candidate with explicit proof gates.
