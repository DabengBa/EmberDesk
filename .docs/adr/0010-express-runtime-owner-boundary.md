...
The former Hono route-island experiment for `POST /api/moving-ui/save` was removed by ADR-0013. It did not justify retaining a second route framework.
...
4. The rollback surface for a full runtime switch would be materially larger than the benefit currently proven.
...
- Route-local validation remains available through direct Express modules without a second framework.
...
```bash
pnpm --dir tests run test:unit -- express5-route-compatibility.test.js plugin-loader.test.js server-startup-profiler.test.js startup-critical-path.test.js startup-deferred-tasks.test.js startup-loader.test.js --runInBand
