...
- guarded React character and group authoring surfaces inside the existing right drawer, with field-local validation, one visible dirty/save state, keyboard-reachable group member reorder controls, and legacy popup/file-write seams preserved underneath as compatibility boundaries
