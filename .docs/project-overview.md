...
- React migration: `/login`, `/setup`, `/settings`, Character Library, Main Chat Message List, World Info, Extensions Host, Character Authoring, Group Authoring, and the same-entry `/` shell have entered React migration. The global Background Library has since been retired; active chat backgrounds and avatar/persona media remain separate retained contracts. The final destination for retained migrated surfaces is React as sole runtime owner, not permanent guarded fallback; see [ADR-0012](adr/0012-react-migrated-surface-legacy-retirement.md). The program remains same-entry and does not introduce `/workspace-next`.
- Workspace panel and shell bootstrap: the current payload and guarded bundles remain the migration baseline while retirement work is unfinished for retained surfaces. The global Background Library panel, gallery, Expressions/waifu controls, and global sprite persistence are retired; active chat background settings/metadata, static `backgrounds/*` compatibility, avatar/persona media, and group `hideMutedSprites` remain supported separately. Release rollback is a previous application version rather than a live legacy path.
...
- an opt-in same-entry React workspace chrome for current context, shell status, AI Config, Formatting, Character Library, World Info, Extensions, and Settings navigation, with Settings / AI Config / Formatting navigating to the React `/settings` sole-owner route (missing build fails closed), visible status copy kept in short human-readable phrases, mobile status chrome hidden when width is tight, and remaining panel entries exposing only transient active/status coordination instead of persistent shell-owned drawer state; the global Background Library entry is retired
