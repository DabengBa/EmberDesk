...
- character metadata, chat stats, full World Info, settings, secrets, and managed media are delivered authority slices; the shared storage control plane is delivered for slice registration, isolation, operator status, and backup readiness; remaining active packages migrate personas, extension state, chat messages, and finally vector source/chunk catalogs
