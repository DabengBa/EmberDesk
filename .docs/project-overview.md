...
- React migration: `/login`, `/setup`, `/settings`, Character Library, Main Chat Message List, World Info, Extensions Host, Character Authoring, and the same-entry `/` shell have entered React migration. The global Background Library and group chat/group-authoring surfaces have since been retired and remain historical compatibility markers only; active chat backgrounds and avatar/persona media remain separate retained contracts. The final destination for retained migrated surfaces is React as sole runtime owner, not permanent guarded fallback; see [ADR-0012](adr/0012-react-migrated-surface-legacy-retirement.md). The program remains same-entry and does not introduce `/workspace-next`.
- Workspace panel and shell bootstrap: the current payload and guarded bundles remain the migration baseline while retirement work is unfinished for retained surfaces. The global Background Library panel, gallery, Expressions/waifu controls, global sprite persistence, and the group-chat/group-authoring surfaces are retired; active chat background settings/metadata, static `backgrounds/*` compatibility, avatar/persona media, and group `hideMutedSprites` remain supported separately. Each migrated surface must replace its flag-off/build-failure branch, hidden host, and legacy action owner before declaring completion; release rollback is a previous application version rather than a live legacy path.
...
- React Character Authoring remains the sole owner of character create/edit fields inside the existing right drawer, with field-local validation, one visible dirty/save state, create/edit destructive-action separation, single visible owner behavior over the retained legacy compatibility host, and legacy popup/file-write seams preserved only as compatibility boundaries. Group authoring is retired.
- an opt-in same-entry React workspace chrome for current context, shell status, AI Config, Formatting, Character Library, World Info, Extensions, Settings, and Character Authoring navigation; the global Background Library entry, Group Chats, and multi-character authoring are retired and are not shell entries. Settings / AI Config / Formatting navigate to the React `/settings` sole-owner route (missing build fails closed), visible status copy stays in short human-readable phrases, mobile status chrome hides when width is tight, and remaining panel entries expose only transient active/status coordination instead of persistent shell-owned drawer state
...
- active chat backgrounds and chat-local background metadata, plus retained avatar/persona images and compatibility paths
