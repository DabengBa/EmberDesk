...
- canonical reads only treat `chat_size` / `date_last_chat` as DB authority when `features.storage.canonicalSqlite.chatStats=true`; otherwise they keep those stats on the file-backed path until the Phase 4 cutover finishes
