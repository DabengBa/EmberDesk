...
- character metadata, chat stats, full World Info, settings, and secrets are delivered authority slices; the shared storage control plane is delivered for slice registration, isolation, operator status, and backup readiness; remaining active packages migrate managed media, personas, extension state, chat messages, and finally vector source/chunk catalogs
