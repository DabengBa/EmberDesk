...
- an opt-in same-entry React workspace chrome for current context, shell status, AI Config, Formatting, Character Library, World Info, Backgrounds, Extensions, and Settings navigation, with Settings falling back to the legacy drawer when the React settings page is unavailable
