...
- React migration: `/login`, `/setup`, `/settings`, Character Library, Main Chat Message List, World Info, Background Library, Extensions Host, Character Authoring, and the same-entry `/` shell have entered React migration. Their final destination is now React as sole runtime owner, not permanent guarded fallback; retired group-chat and group-authoring paths remain historical compatibility markers only. See [ADR-0012](adr/0012-react-migrated-surface-legacy-retirement.md). The program remains same-entry and does not introduce `/workspace-next`.
...
- React Character Authoring remains the sole owner of character create/edit fields inside the existing right drawer, with field-local validation, one visible dirty/save state, create/edit destructive-action separation, and legacy popup/file-write seams preserved only as compatibility boundaries.
- an opt-in same-entry React workspace chrome for current context, shell status, AI Config, Formatting, Character Library, World Info, Backgrounds, Extensions, Settings, and Character Authoring navigation; Group Chats and multi-character authoring are retired and are not shell entries. Settings / AI Config / Formatting navigate to the React `/settings` sole-owner route (missing build fails closed), visible status copy stays in short human-readable phrases, mobile status chrome hides when width is tight, and remaining panel entries expose only transient active/status coordination instead of persistent shell-owned drawer state
