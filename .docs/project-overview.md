...
- React migration: `/login`, `/setup`, `/settings`, Character Library, Main Chat Message List, World Info, Extensions Host, Character Authoring, Group Authoring, and the same-entry `/` shell have entered React migration. The former Background Library management surface was retired on 2026-09-15; its React panel, legacy owner, feature flag, and management commands are gone while background URL/render/settings compatibility remains. For the remaining migrated surfaces, the final destination is React as sole runtime owner, not permanent guarded fallback; see [ADR-0012](adr/0012-react-migrated-surface-legacy-retirement.md). The program remains same-entry and does not introduce `/workspace-next`.
...
- background URL/render/settings compatibility, `/backgrounds/*` static reads, and historical/default background assets; background management is retired rather than a current workspace capability
- extension surfaces inside the main shell
