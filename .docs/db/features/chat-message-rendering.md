...
- **Success**: stored or finalized messages appear as readable `.mes_text` inside stable `.mes[mesid]` rows, and safe visible rows may carry a React owner marker without changing the user's reading flow.
