...
- Editing, streaming, and extension-mutated rows remain on the same React-owned `.mes[mesid]` lifecycle with `preserveLiveContent` and mutation zones rather than remounting as a second owner. Structurally unsupported rows stay unowned rather than restoring a product dual-path legacy renderer.
...
Main-chat rendering is a completed final-wave retirement surface. React owns all supported row families, rich body, actions/swipes, extension mutation zones, and long-chat windowing. Documented formatting exports may remain as thin delegates to the framework-neutral render service; product dual-owner list/windowing flags are retired.
