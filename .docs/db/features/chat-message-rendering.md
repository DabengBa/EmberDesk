...
- The current Phase 3 Sprint 5 React migration boundary still does not replace those rows with a new JSX row tree. Instead, the guarded React main-chat island keeps the visible message window and load-more node ordered inside `#chat`, validates Zod-backed rich-body snapshots for visible finalized rows, claims those rows only through hidden per-row owner markers when the bridge payload is present and valid, uses TanStack Virtual only as a headless measurement/snapshot/restore controller for current-session reading-position recovery, and consumes a hidden Zod-backed `generationControl` snapshot for visible generation control phase.
...
- The hidden `generationControl` bridge does not make React the owner of token append, provider transport, user stop, final retry, composer input, or slash-command execution. It only lets the guarded React controller observe the control phase without changing message row identity.
