...
- Stored chat history continues to open as one complete, ordered conversation for the existing renderer and load-more flow. Persistence migration must preserve message order, swipes, metadata, and readable content without changing row ownership or adding server pagination.
