...
- Editing rows, active streaming rows, snapshot parse failures, or structurally unsafe rows must fail closed to legacy ownership instead of leaving the user with a half-React or half-legacy row.
...
- When the guarded React main-chat flag is enabled, supported visible `submitComposer` and `continueLast` requests may temporarily hand the active assistant row's token append and finalization path to a React-owned visible transport mutation. That cutover must still preserve a single assistant row, keep `.mes[mesid]` identity stable, and fail closed per request back to legacy when the request is unsupported or the bridge is unsafe.
- Unsupported visible transport kinds such as `retry/regenerate/swipe`, along with non-OpenAI, group, dry-run, or nested-visible generation paths, remain legacy-owned and continue to use the ordinary `Generate()` / `StreamingProcessor` rendering path.
...
- **Success**: stored or finalized messages appear as readable `.mes_text` inside stable `.mes[mesid]` rows, safe visible rows may carry a React owner boundary without changing the user's reading flow, and supported send/continue streaming completes on the same assistant row instead of creating a duplicate transport row.
