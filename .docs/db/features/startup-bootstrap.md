...
- The overlay disappears when the main shell is usable for real interaction, even if secondary surfaces such as extensions are still warming. Active chat background application is chat context, not a background-library panel.
- Deferred extension failures surface in their own panel areas instead of returning the whole workspace to global loading.
...
- As a user encountering a secondary surface failure during startup, from the usable workspace open the affected extension surface or apply an active chat background; EmberDesk must keep the main shell usable and show local retry, refresh, or error behavior, and failure is returning to global loading or leaving a blank surface with no recovery cue.
...
- A retained active-background application failure blocks the whole workspace.
- An extension load failure blocks the whole workspace.
...
- Active chat background application and its metadata belong to [Chat Workspace](page.chat_workspace); the retired global Background Library has no startup-loading owner.
