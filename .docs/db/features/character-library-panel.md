...
- After an ordinary unfiltered single-character delete, the panel keeps the remaining visible character rows aligned with the shifted `characters` array by rewriting `data-chid`, legacy `chid`, and `CharID${chid}` values instead of forcing a whole-list redraw.
- If the delete occurs in a complex browsing state such as active search/filtering, bogus-folder drilldown, bulk edit mode, multi-delete, or an in-flight list print, the panel may fall back to the full list refresh path to preserve correctness.
