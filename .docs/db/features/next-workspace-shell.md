...
- Registry entries route Settings, AI Config, and Formatting to React Settings. Character Library, World Info, and Extensions use declared child-slot contracts. Character creation and editing remain Character Library actions rather than a separate shell entry. Background Library management is retired and is not a shell entry.
...
- Primary entries for AI Config, Formatting, Character Library, World Info, Extensions, Settings, are reachable by role/name and use the existing workspace behavior or the shared Settings overlay. Background management has no current navigation entry.
...
- Local empty/error recovery actions stay scoped to the affected surface instead of blocking the full workspace. Current examples include opening Character Library from an empty main-chat state, retrying or continuing a failed visible generation, importing or refreshing World Info, and opening Manage or retrying Extras connection from Extensions.
...
- Opening Character Library, World Info, or Extensions from the chrome must succeed on the first click without freezing the visible workspace or requiring a second click to settle panel state.
...
- `feature.next_workspace_shell.primary_navigation`: the AI Config, Formatting, Character Library, World Info, Extensions, Settings entry set; Background Library management has no entry.
...
- As a user opening AI Config, Formatting, Character Library, World Info, Extensions, Settings, from the chrome, use the named entry; EmberDesk must open the established surface or route while preserving protected DOM and extension locations, and failure is a visible button that does nothing or clears legacy panel content.
...
- Panel dock markers such as `data-workspace-shell-panel-entry` and `data-workspace-shell-panel-active` support proof that React owns coordination state without taking over AI configuration, formatting, settings, World Info, Extension, or character-row semantics. Character and group authoring behavior remains owned by the dedicated authoring surfaces rather than the shell registry itself.
...
- AI Config, Formatting, Character Library, World Info, Extensions, Settings, and Character Authoring retain their own feature contracts after the shell opens them. Background URL/render/settings compatibility remains outside shell navigation.
...
- Background URL/render/settings reads remain a compatibility concern outside this shell feature; the retired Background Library management surface has no shell lifecycle or child slot.
