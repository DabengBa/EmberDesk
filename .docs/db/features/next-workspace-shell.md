...
- Registry entries route Settings, AI Config, and Formatting to React Settings. Character Library, World Info, Extensions, and Main Chat use declared child-slot contracts. The former global Background Library entry is retired; active chat backgrounds remain chat context rather than a shell panel.
...
- Primary entries for AI Config, Formatting, Character Library, World Info, Extensions, and Settings are reachable by role/name and use the existing workspace behavior or the shared Settings overlay. The retired Background Library has no replacement navigation entry.
...
- Local empty/error recovery actions stay scoped to the affected surface instead of blocking the full workspace. Current examples include opening Character Library from an empty main-chat state, retrying or continuing a failed visible generation, importing or refreshing World Info, and opening Manage or retrying Extras connection from Extensions. Active background fallback is handled by chat metadata, not a library refresh workflow.
...
- Opening Character Library, World Info, or Extensions from the chrome must succeed on the first click without freezing the visible workspace or requiring a second click to settle panel state. The retired Background Library has no shell entry.
...
- `feature.next_workspace_shell.primary_navigation`: the AI Config, Formatting, Character Library, World Info, Extensions, Settings entry set. The former Backgrounds entry is retired.
...
- Panel dock markers such as `data-workspace-shell-panel-entry` and `data-workspace-shell-panel-active` support proof that React owns coordination state without taking over AI configuration, formatting, settings, World Info, Background, Extension, or character-row semantics. Character authoring behavior remains owned by its dedicated surface rather than the shell registry itself.
...
- AI Config, Formatting, Character Library, World Info, Extensions, Settings, and Character Authoring retain their own feature contracts after the shell opens them. Active chat backgrounds and avatar/persona media remain workspace capabilities; the global Background Library is retired.
