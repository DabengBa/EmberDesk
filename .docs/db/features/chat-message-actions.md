...
- The current Phase 3 Sprint 8 main-chat migration still does not move action ownership into React. Even when `features.react.panels.mainChatMessageList` is enabled, message actions, edit buttons, swipe affordances, reasoning action buttons, failure retry affordances, and other message-row controls remain attached to the legacy outer row shell and its existing delegated handlers. React may observe a hidden message-action snapshot for rows that already expose the protected action shell and may append a hidden owner marker inside that same action area, but it does not add visible buttons, replace handlers, reorder the existing expanded-actions controls, or replace `.generation_failure_retry`.
