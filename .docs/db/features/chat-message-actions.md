...
- The current Phase 3 Sprint 5 main-chat migration does not move action ownership into React. Even when `features.react.panels.mainChatMessageList` is enabled, message actions, edit buttons, swipe affordances, reasoning action buttons, failure retry affordances, and other message-row controls remain attached to the legacy outer row shell and its existing delegated handlers. React may consume a hidden generation-control snapshot, but it does not replace `.generation_failure_retry`.
