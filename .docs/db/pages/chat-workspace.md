...
- **Main-chat migration state**: the main chat workspace can enable a guarded React message-list island behind `features.react.panels.mainChatMessageList`. In the current Sprint 5 boundary the React bundle mounts a hidden host inside `#chat`, preserves the direct-child `.mes[mesid]` rows plus `#show_more_messages` ordering, validates finalized rich-body snapshots for visible non-editing rows, and adds per-row hidden owner markers only when that bridge state is present and structurally valid. In the same current-page session it also records per-chat reading-position snapshots, including expanded long-chat history windows, and on return can ask legacy load-more to reopen the required window before restoring the prior reading region. React now also consumes a Zod-validated hidden `generationControl` payload for the visible generation control phase (`idle`, `streaming`, `recoveringPrimary`, `recoveringFallback`, `stopped`, `completed`, or `error`) while `public/script.js` keeps ownership of `Generate()`, `StreamingProcessor`, token append, stop, auto-recovery status, final retry, `#mes_continue`, and `.generation_failure_retry`. The outer message row shell, formatter chain, code-block/media/file live DOM, provider transport, input, slash-command pause/continue/abort, and message-action logic remain legacy-owned, and Sprint 3 did not bring back a separate `#jump_to_latest_message` control. If the flag is off, the bundle cannot mount, a bridge payload is unsafe, or a row does not satisfy the guarded snapshot path, the chat area stays fully legacy-rendered or falls back to the normal legacy open result.
