...
- **Main-chat migration state**: the main chat workspace can enable a guarded React message-list island behind `features.react.panels.mainChatMessageList`. In the current Phase 3B Sprint 1 boundary, the React bundle still mounts a hidden host inside `#chat`, preserves direct-child `.mes[mesid]` rows plus `#show_more_messages` ordering, records per-chat reading-position snapshots, and consumes Zod-validated hidden `generationControl`, `streamingTransport`, `composer`, and `slashCommand` payloads. It now also validates `messageRowSnapshots` for safe stored or finalized non-editing rows and marks those rows with a visible React owner boundary on the existing `.mes[mesid]` root while keeping the same row identity, readable `.mes_text`, swipe wrappers, `.mes_buttons`, and long-chat load-more semantics. Editing rows, active streaming rows, invalid snapshots, or structurally unsafe rows fail closed to legacy ownership inside the same chat window. `public/script.js` still owns `Generate()`, `StreamingProcessor`, provider transport, token append, stop, auto-recovery status, final retry, `#mes_continue`, `.generation_failure_retry`, `#send_textarea`, `#send_but`, slash parser/registry/executor, autocomplete DOM, pause/continue/abort controls, and all visible message actions. The formatter chain, code-block/media/file live DOM, provider transport, visible composer, slash command execution, and visible action logic remain legacy-owned. Sprint 3 did not bring back a separate `#jump_to_latest_message` control. If the flag is off, the bundle cannot mount, a bridge payload is unsafe, or a row does not satisfy the guarded snapshot path, the chat area stays fully legacy-rendered or falls back to the normal legacy open result.
