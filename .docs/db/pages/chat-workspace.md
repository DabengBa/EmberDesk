...
- **React chrome navigation state**: When the React workspace chrome is mounted, AI Config, Formatting, Character Library, World Info, Backgrounds, Extensions, Settings, and Group Chats are the visible primary navigation entries. AI Config opens the React [Settings](page.settings) surface as an in-workspace overlay on the Providers tab; Formatting opens the same overlay on Advanced; Settings opens the same overlay on General. Direct `/settings` remains available as a deep-link full-page mount. Character Library owns the create/edit entry points for React character forms; Group Chats keeps its right-drawer entry point, and React remains the sole owner for group and character create/edit fields and direct save commands. Missing workspace-panels builds fail closed; previous-version deploy is the rollback path.
