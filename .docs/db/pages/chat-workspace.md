...
4. **Supporting panels and drawers**: secondary UI surfaces for World Info and extensions, while the former global Background Library panel is retired. Active chat background selection remains part of chat context, not a library drawer.
...
- **Settings navigation state**: the React workspace shell Settings, AI Config, and Formatting entries open the same in-workspace [Settings](page.settings) overlay on General, Providers, and Advanced. Direct `/settings?tab=...` remains the full-page deep-link path; these entries do not reopen legacy settings drawers as product fallbacks.
- **Shell ownership state**: `/` always starts the React shell. Shell flags, inline workspace feature bootstrap, and same-version legacy shell fallback are retired; operational rollback uses a prior release.
- **Child-slot state**: Character Library, World Info, Extensions, Character Authoring, and the Main Chat regions retain stable protected DOM only as declared child content or compatibility hosts. Character Authoring is entered from Character Library's create/edit actions, not from shell navigation. Their legacy classes do not own shell navigation or pin state.
...
- **Active background state**: the workspace can apply a global active background setting and a chat-specific `custom_background`/`chat_backgrounds` override. Removing the chat override returns to the global active background without turning either state into a global gallery.
- **Retired Background Library state**: the workspace does not expose a Backgrounds panel, global background catalog, expressions/waifu controls, or global sprite persistence. Static background paths, avatar/persona images, and retained thumbnail behavior remain separate compatibility capabilities.
- **Supporting-panel migration state**: World Info and Extensions continue to open from their existing workspace entries. Their visible React ownership and protected compatibility slots remain documented by their own features. No Background Library fallback panel or same-entry replacement is created.
- **Workspace panel dock coordination state**: When the same-entry React chrome is mounted, Character Library, World Info, and Extensions entries publish transient active-panel/dock state. Character creation and editing are subflows of Character Library, rather than a second top-level entry. The chrome shows only the selected assistant, or character name for current context, using `EmberDesk` for the default assistant; it does not repeat chat type, chat ID, message count, temporary-chat state, workspace-ready, or panel-ready/status copy. The active entry remains the visible confirmation that a panel is open; errors and recovery actions remain inside the affected panel. Clicking the active entry closes the established surface when the owner reports that it closed; clicking it again reopens it from the same entry. Settings, AI Config, and Formatting use the same transient dock state while presenting the shared [Settings](page.settings) owner as an independent overlay rather than a drawer or route transition. The latest manual open, refocus, and pinned-drawer intent is remembered only in the current browser page session, so a refresh or remount in that same session does not immediately discard the user's last panel choice while still avoiding a new long-term preference.
- **Main-chat owner state**: React is the sole runtime owner of the main-chat message list inside the existing `#chat` surface. `app/stores/main-chat-store.ts` owns the typed message snapshot, ordering, visible window, generation/streaming snapshot shape, and transient row state; React emits direct-child `.mes[mesid]` rows, `#show_more_messages` ordering, reading-position restoration, swipe wrappers, `.mes_buttons`, reasoning shells, and the documented extension mutation hosts. There is no product flag that restores a dual-owner legacy list renderer; missing workspace-panels builds fail closed with a visible build error, and rollback is previous-version deploy. React visible rows call named commands for edit/copy/delete/move/retry/swipe, reasoning expand/copy/edit/delete/collapse, and composer generation entry points. Reasoning row commands persist `chat[].extra.reasoning` and emit supported events without reading message DOM; parsing/templates and stream-state behavior remain compatibility-owned. The retained `#send_textarea` / `#send_but` controls are still the composer DOM owner for input value and legacy integration; their React command capture is not yet the final typed composer write owner. One framework-neutral generation service owns request assembly, streaming lifecycle, stop, bounded recovery, fallback attempts, and final results for visible direct, provider-specific, dry-run, nested, quiet, and background requests. The compatibility `Generate()` entry continues to accept existing callers but delegates to that service. Quiet and background helpers remain non-visible, create no assistant row, return generated text, and do not auto-retry. Framework-neutral `chat-message-render-service` produces formatted HTML without owning DOM insertion; React rows consume that service for supported lifecycle states. Compatibility aliases such as `printMessages` / `showMoreMessages` / `updateMessageElement` may remain as thin adapters for callers, but they are not product dual-owner list/windowing surfaces. The hidden controller retains layout, local-status, windowing, and row-lifecycle diagnostics. There is no separate `#jump_to_latest_message` control.
- **Main-chat reading-position state**: after the user scrolls within a chat or expands older history, leaving that chat and returning in the same browser page session should restore the prior reading region. If the saved anchor can no longer be reopened safely, EmberDesk falls back to the normal open result instead of leaving the chat in a half-restored state.
...
- **Retired group chat state**: the workspace no longer exposes Group Chats navigation, group authoring, multi-character generation, or group rows in the character library. Stale `/api/groups/*` and chats group write paths return JSON `410 Gone` with `group_chat_feature_removed`. Existing on-disk `groups/` and `group chats/` files are left untouched and are not auto-purged. `context.groupId` remains empty.
...
- **React chrome navigation state**: When the React workspace chrome is mounted, AI Config, Formatting, Character Library, World Info, Extensions, and Settings are the visible primary navigation entries. AI Config opens the React [Settings](page.settings) surface as an in-workspace overlay on the Providers tab; Formatting opens the same overlay on Advanced; Settings opens the same overlay on General. Direct `/settings` remains available as a deep-link full-page mount. Character Library owns the create/edit entry points for React character forms; React remains the sole owner for character create/edit fields and direct save commands; group authoring and the global Background Library are retired. Missing workspace-panels builds fail closed; previous-version deploy is the rollback path.
...
- **First-open panel state**: Opening World Info, Character Library, or Extensions from the React chrome should succeed on the first click without freezing the visible workspace. The retired Background Library has no navigation entry; active background application remains available through chat context and metadata.
- **Global compatibility export state**: `globalThis.SillyTavern`, `eventSource` / `event_types`, and `@sillytavern/*` remain supported extension contracts. The workspace keeps their established behavior while internal owners are modernized; `/lib.js` stays the preferred shared browser utility surface, and `__emberDeskReactCompatibilityBridge` stays internal-only.
...
- **Streaming, composer, slash, and quiet-helper bridge state**: the composer keeps the established `#send_textarea` / `#send_but` selectors and routes submit, stop, continue, and regenerate through named command capture while the typed snapshot is projected into React. The final store-owned composer write path is still a separate retirement item. React renders the typed slash autocomplete and paused/aborted/error status shell, while the slash parser, registry, executor, and public exports remain compatibility-owned; streaming diagnostics remain available for tests; quiet/background requests retain their non-visible, no-row, return-string behavior without separate owner markers.
...
- **Mobile panel dock state**: On narrow viewports, React navigation remains icon reachable without panel-status copy consuming chrome width or covering the composer; drawer content and protected extension/message surfaces remain reachable through the existing mobile layout rules.
...
- From this shell, users can move between the character library, chat context, World Info, and extension surface without leaving [Chat Workspace](page.chat_workspace). The former global Background Library does not introduce a replacement route or panel.
- React [Settings](page.settings) is the sole product owner for general, provider, UI, and advanced settings; shell and legacy direct top-bar Settings / AI Config / API Connections / Formatting entries open the same Settings surface as an in-workspace overlay instead of leaving chat or opening their old forms. Specialized World Info, Extensions, active chat backgrounds, and avatar/persona media remain on this workspace.
