...
4. **Supporting panels and drawers**: secondary UI surfaces for World Info, extensions, and related workspace tools; background URLs and settings remain compatible without a background-management panel.
...
- **Child-slot state**: Character Library, World Info, Extensions, Character Authoring, and the Main Chat regions retain stable protected DOM only as declared child content or compatibility hosts. Character Authoring is entered from Character Library's create/edit actions, not from shell navigation. Their legacy classes do not own shell navigation or pin state. Background Library management is retired and has no child slot or shell entry.
...
- **Degraded state**: if a supporting surface such as extensions fails to load, the core shell can still remain usable and the affected panel shows local retry or follow-up refresh behavior.
...
- **Supporting-panel migration state**: World Info and Extensions continue to open from the existing workspace drawer entries. World Info and Extensions are the active React panel surfaces on those same entries; their visible workbench and host controls remain documented by their own features. Background Library management is retired: the workspace exposes no Backgrounds entry, drawer, gallery, React panel, or legacy management host. Existing background URL/render/settings compatibility remains available independently of workspace navigation.
- **Workspace panel dock coordination state**: When the same-entry React chrome is mounted, Character Library, World Info, and Extensions entries publish a transient active-panel/dock state. Character creation and editing are subflows of Character Library, rather than a second top-level entry. The chrome shows only the selected assistant, or character name for current context, using `EmberDesk` for the default assistant; it does not repeat chat type, chat ID, message count, temporary-chat state, workspace-ready, or panel-ready/status copy. The active entry remains the visible confirmation that a panel is open; errors and recovery actions remain inside the affected panel. Clicking the active entry closes the established surface when the legacy owner reports that it closed; clicking it again reopens the surface from the same entry. Settings, AI Config, and Formatting use the same transient dock state while presenting the shared [Settings](page.settings) owner as an independent overlay rather than a drawer or route transition. The latest manual open, refocus, and pinned-drawer intent is remembered only in the current browser page session, so a refresh or remount in that same session does not immediately discard the user's last panel choice while still avoiding a new long-term preference. The same transient path also carries drawer `pinnedOpen` facts as `locked` / `pinned` metadata for compatibility snapshots and focused proof, but the visible shell chrome does not render separate pinned/locked badges. Flag-off, missing-container, bundle-load-failed, or mount-failed paths for remaining panel surfaces still open the established fallback surface from the same workspace entry and must not create an empty migration host.
...
- **First-open panel state**: Opening World Info, Character Library, or Extensions from the React chrome should succeed on the first click without freezing the visible workspace; the action should settle locally to the chosen panel instead of stalling the whole shell.
