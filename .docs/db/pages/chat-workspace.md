...
- **React chrome navigation state**: When the React workspace chrome is mounted, AI Config, Formatting, Character Library, World Info, Backgrounds, Extensions, and Settings are the visible primary navigation entries. AI Config and Formatting open the existing `#left-nav-panel` and `#AdvancedFormatting` drawers, Settings opens the React [Settings](page.settings) route only when that page flag is enabled, and otherwise opens the existing `#user-settings-block` drawer in this workspace.
