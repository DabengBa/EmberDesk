...
- **Main-chat migration state**: the main chat workspace can enable a guarded React message-list island behind `features.react.panels.mainChatMessageList`. In the current Sprint 2 boundary the React bundle mounts a hidden host inside `#chat`, preserves the direct-child `.mes[mesid]` rows plus `#show_more_messages` ordering, validates finalized rich-body snapshots for visible non-editing rows, and adds per-row hidden owner markers only when that bridge state is present and structurally valid. The outer message row shell, formatter chain, code-block/media/file live DOM, streaming, input, and message-action logic remain legacy-owned. If the flag is off, the bundle cannot mount, or a row does not satisfy the guarded snapshot path, the chat area stays fully legacy-rendered.
