...
- **Child-slot state**: Character Library, World Info, Backgrounds, Extensions, Group Authoring, Character Authoring, and the Main Chat regions retain stable protected DOM only as declared child content or compatibility hosts. Their legacy classes do not own shell navigation or pin state.
