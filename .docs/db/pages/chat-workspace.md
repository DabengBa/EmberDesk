...
- **Supporting-panel migration state**: World Info, Backgrounds, and Extensions continue to behave as the existing workspace drawer surfaces. Guarded World Info, Background Library, and Extensions Host status surfaces may appear only when their migration flags are enabled; when a flag is off, EmberDesk does not add an empty migration host to that legacy panel. These status surfaces do not replace global activation, import, regex, prompt activation, world-book deletion, background upload/delete/rename/select/slash behavior, extension settings mount points, wand-menu entries, or extension loading behavior.
