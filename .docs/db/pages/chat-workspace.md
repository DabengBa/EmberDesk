...
- **Main-chat migration state**: the main chat workspace can enable a guarded React message-list island behind `features.react.panels.mainChatMessageList`. In the current Phase 3 guarded hidden-island boundary, the React bundle mounts a hidden host inside `#chat`, preserves direct-child `.mes[mesid]` rows plus `#show_more_messages` ordering, validates finalized rich-body snapshots for visible non-editing rows, derives hidden message-action snapshots from the existing message-action shell, and adds hidden owner markers only when bridge states are present and structurally valid. In the same current-page session it records per-chat reading-position snapshots, including expanded long-chat history windows, and can ask legacy load-more to reopen the required window before restoring the prior reading region. React also consumes Zod-validated hidden `generationControl`, `streamingTransport`, `composer`, and `slashCommand` payloads: generation and streaming markers report visible control phase, terminal transport state, active message id, fallback attempt, token/chunk observation, composer length/focus/sendability, and slash active/autocomplete/execution state without exposing prompt text or command arguments. `public/script.js` still owns `Generate()`, `StreamingProcessor`, provider transport, token append, stop, auto-recovery status, final retry, `#mes_continue`, `.generation_failure_retry`, `#send_textarea`, `#send_but`, slash parser/registry/executor, autocomplete DOM, pause/continue/abort controls, and all visible message actions. The outer message row shell, formatter chain, code-block/media/file live DOM, provider transport, visible composer, slash command execution, and visible action logic remain legacy-owned; React only appends hidden markers after protected legacy controls. Sprint 3 did not bring back a separate `#jump_to_latest_message` control. If the flag is off, the bundle cannot mount, a bridge payload is unsafe, or a row does not satisfy the guarded snapshot path, the chat area stays fully legacy-rendered or falls back to the normal legacy open result.
...
- **Generation control bridge state**: when the guarded React main-chat flag is enabled, the hidden React controller can read the current visible generation control phase without becoming the provider transport owner. User stop still avoids automatic retry, recovery status stays outside `.mes_text`, and final failure keeps the existing retry affordance on the preserved assistant row.
- **Streaming, composer, and slash bridge state**: when the guarded React main-chat flag is enabled, the hidden controller can also read streaming transport metadata, composer metadata, and slash-command metadata. Streaming transport markers observe legacy token/chunk counts and terminal stop/error/completion snapshots without owning provider requests. Composer markers expose length, focus, disabled/generating, sendability, and active context without copying raw prompt text. Slash markers expose active/query length, autocomplete visibility, execution, pause, abort, and error label without copying full command text or arguments.
