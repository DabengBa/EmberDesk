...
describe('Background Library retirement baseline', () => {
    ...
    test('removes React, legacy, feature, store, command, and bridge management references', () => {
        ...
        for (const command of ['lockbg', 'unlockbg', 'autobg']) {
            expect(backgroundsSource).not.toContain(`name: '${command}'`);
        }
    });

    test('keeps /bg as a direct background setter without the catalog enum provider', () => {
        const slashCommandsSource = read('public/scripts/slash-commands.js');

        expect(slashCommandsSource).toContain("name: 'bg'");
        expect(slashCommandsSource).toContain('callback: setBackgroundCallback');
        expect(slashCommandsSource).not.toContain('enumProvider: commonEnumProviders.backgrounds');
    });

    test('preserves background static reads, rendering, settings, and URL helpers', () => {
        ...
        expect(backgroundsSource).toContain('export function loadBackgroundSettings(settings)');
        expect(backgroundsSource).toContain('settings.background');
        expect(backgroundsSource).toContain('export function getBackgroundPath(fileUrl)');
        expect(backgroundsSource).toContain('function generateUrlParameter(bg, isCustom)');
        expect(backgroundsSource).toContain("$('#bg1')");
    });
