...
describe('character list structure', () => {
    ...
    test('keeps generated character row selectors and active-state hooks stable', () => {
        ...
        expect(scriptSource).toContain('class="character_select entity_block');
        expect(scriptSource).toContain('data-chid="${id}" chid="${id}" id="CharID${id}"');
        expect(scriptSource).toContain('class="ch_fav"');
        expect(scriptSource).toContain('class="tags tags_inline"');
